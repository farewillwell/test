...
def train_step(
    ...
    def loss_fn(
        ...
        if config.use_awbc and adv is not None:
            ...
            log_w = jnp.clip(config.awbc_beta * adv, -config.awbc_log_weight_clip, config.awbc_log_weight_clip)
            w = jnp.exp(log_w)
            w = jnp.clip(w, config.awbc_weight_min, config.awbc_weight_max)
            w = w / (jnp.mean(w) + 1e-8)

            loss = jnp.sum(per_sample_loss * w) / (jnp.sum(w) + 1e-8)
        else:
    ...
    loss, grads = nnx.value_and_grad(loss_fn, argnums=diff_state)(model, train_rng, observation, actions,adv)
...
def main(config: _config.TrainConfig):
    ...
    logging.info(f"JAX compilation cache: {jax.config.jax_compilation_cache_dir}")
