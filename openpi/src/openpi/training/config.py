...
@dataclasses.dataclass(frozen=True)
class LeRobotLiberoDataAWBCConfig(DataConfigFactory):
    extra_delta_transform: bool = False
...
class TrainConfig:
    ...
    # Optional exact checkpoint directory. If set, skips the default
    # checkpoint_base_dir / config_name / exp_name nesting.
    checkpoint_dir_override: str | None = None
    # Optional directory that should contain a directly loadable inference checkpoint.
    # When set, training checkpoints are still managed under checkpoint_dir, and the
    # latest saved step is published here as {params, assets, train_state, ...}.
    published_checkpoint_dir: str | None = None
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(action_horizon=LIBERO_REPLAN_STEPS),
